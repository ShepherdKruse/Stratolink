...
import { peekLaunchToken } from '@/lib/actions/launch-kit';
...
export default function ActivateDeviceInner({ params }: PageProps) {
    ...
    const launchKRef = useRef<string | null>(null);

    useEffect(() => {
        launchKRef.current = searchParams.get('k');
    }, [searchParams]);
    ...
    useEffect(() => {
        ...
        let cancelled = false;
        peekLaunchToken(deviceId, k).then((r) => {
            if (!cancelled) setSkipPin(r.skipPin);
        });
        return () => {
            cancelled = true;
        };
    ...
    const canProceedFromStep1 = (skipPin || pin.length === 6) && name.trim().length > 0;
    ...
    const handleLaunch = async () => {
        ...
        const launchToken = launchKRef.current;
    ...
    return (
        <div className="min-h-screen bg-[#1a1a1a] text-[#e5e5e5]">
            <div className="fixed top-0 left-0 right-0 h-px bg-[#333] z-50">
                <div className="h-full bg-[#4a90d9] transition-all duration-300" style={{ width: `${(step / 4) * 100}%` }} />
            </div>

            <div className="fixed top-3 right-3 z-40">
                <div className="bg-[#1a1a1a]/95 backdrop-blur-md border border-[#333] px-3 py-1.5">
                    <span className="text-[10px] font-mono text-[#666] uppercase tracking-wider">Step {step} / 4</span>
                </div>
            </div>
                            ...
                            <div className="space-y-6">
                                <div className="border-b border-[#333] pb-4">
                                    ...
                                    <p className="text-[11px] text-[#666] font-mono">
                                        {skipPin ? 'Launch link verified — enter your name to continue' : 'Enter activation credentials'}
                                    </p>
                                </div>

                                {skipPin && (
                                    <div className="rounded border border-emerald-700/50 bg-emerald-950/40 px-3 py-2 text-[11px] text-emerald-200 font-mono">
                                        No PIN needed (one-time link from your QR). This link expires and is cleared after launch.
                                    </div>
                                )}
                                ...
                                {locationError ? (
                                    ...
                                    <div className="space-y-4">
                                        <div className="bg-[#141414] border border-[#333] p-3">
                                            <div className="text-[10px] font-semibold text-[#666] uppercase tracking-wider mb-1">
                                                Launch Location
                                            </div>
                                ...
                                <div className="flex flex-col items-center space-y-6">
                                    <div className="relative">
                                        <svg className="w-56 h-56 transform -rotate-90" viewBox="0 0 100 100">
                                            <circle cx="50" cy="50" r="45" fill="none" stroke="rgba(204, 68, 68, 0.2)" strokeWidth="3" />
                                            <motion.circle
                                                cx="50"
                                                cy="50"
                                                r="45"
